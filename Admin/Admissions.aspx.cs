...
public partial class Admin_Admissions : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string j = "select * from  admissioncategory";
            ds1 = dc.getdata(j);
                ...
                else
                {
                    if (ActionLbl.Text == "Add")
                    {
                        if (Image.HasFile)
                        {
                            ...
                            {
                                Image.SaveAs(Server.MapPath(ConfigurationManager.AppSettings["AdmissionImgPath"]) + Image.FileName);
                                string path = ConfigurationManager.AppSettings["AdmissionImgPath"] + Image.FileName;

                                string zk = "insert into admissions (CreatedOn,Title,Image,Content,CategoryId,IsShow) values('" + current_time.ToShortDateString() + "','" + TitleTb.Text + "','" + path + "','" + DescriptionTb.Text + "','" + CategoryDD.SelectedValue + "'," + IsShowChk.Checked + ")";
                                dc.setdata(zk);
                    ...
                    else
                    {
                        if (Image.HasFile)
                        {
                            ...
                            if (ext.ToLower() == ".png" || ext.ToLower() == ".jpg" || ext.ToLower() == ".jpeg")
                            {
                                ...
                                string zk = "update admissions set CreatedOn='" + current_time.ToShortDateString() + "',Title='" + TitleTb.Text + "',Image='" + path + "',Content='" + DescriptionTb.Text + "',CategoryId='" + CategoryDD.SelectedValue + "',IsShow=" + IsShowChk.Checked + " where id='" + ImageIdLbl.Text + "'";
                                dc.setdata(zk);
                        ...
                        {
                            string zk = "update admissions set CreatedOn='" + current_time.ToShortDateString() + "',Title='" + TitleTb.Text + "',Content='" + DescriptionTb.Text + "',CategoryId='" + CategoryDD.SelectedValue + "',IsShow=" + IsShowChk.Checked + " where id='" + ImageIdLbl.Text + "'";
                            dc.setdata(zk);
                            ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "alert", "alert('Data Updated.');", true);
                            ClearFields();
                        }
    ...
    protected void EditAdmissionLb_Command(object sender, CommandEventArgs e)
    {
        string id = e.CommandArgument.ToString();

        string zkp = "select * from admissions where id='" + id + "'";
    ...
    protected void DeleteAdmissionLb_Command(object sender, CommandEventArgs e)
    {
        string id = e.CommandArgument.ToString();

        string jkp = "delete from admissions where id='" + id + "'";
        dc.setdata(jkp);
        ClearFields();
    }
    ...
    {
        string jk = "select admissions.Id,admissions.Title,admissions.CreatedOn,admissions.Image,admissions.Content,admissions.IsShow,admissioncategory.Category from admissions join admissioncategory on admissions.CategoryId = admissioncategory.Id ORDER BY admissions.CreatedOn DESC";
